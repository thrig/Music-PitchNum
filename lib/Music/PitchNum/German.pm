...
sub pitchname {
  ...
  $note .= (q{'}) x ( $octave - 3 ) if $octave > 3;
  $note .= (q{,}) x ( 3 - $octave ) if $octave < 3;
...
sub pitchnum {
  ...
  if ( $name =~
    m/ (?<note> [A-Ha-h] ) (?<chrome> es(?:es)? | is(?:is)? )? (?<octave> [,]{1,10}|[']{1,10} )? /x
    ) {
    my $octave = $+{octave};
    my $chrome = $+{chrome};
    my $note   = $+{note};
...
Music::PitchNum::German - note name to pitch number utility roles
...
Otherwise, the implementation is Helmholtz-based, or the one typically seen in
C<lilypond>, with accidentals in the Dutch or German style (C<es>, C<is>), and
octave indications using C<,> and C<'>.
...
Returns the pitch number for the given note name, or C<undef> if the note could
not be parsed.
